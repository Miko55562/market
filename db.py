...
def register(name,email,password,public_id):
    ...
    basket=''
    bill = 0
    cur.execute(f"INSERT INTO users VALUES (:id, :name, :email, :password, :elect, :public_id, :basket, :bill);",
    {'id': len(all_emails())+1, 'name': name, 'email': email, 'password': password,
                          'elect': elect, 'public_id':public_id, 'basket': basket, 'bill': bill})
